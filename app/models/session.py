...
class Session(Base):
    ...
    id : Mapped[int] = mapped_column(Integer, primary_key=True, index=True) # Changed to int for clarity
    patient_id : Mapped[str] = mapped_column(String, ForeignKey("patients.document_id"), nullable=False)
    date : Mapped[datetime] = mapped_column(DateTime, nullable=False) # Changed to datetime type for clarity
    ...
    created_at : Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc)) # <--- IMPORTANT CHANGE
